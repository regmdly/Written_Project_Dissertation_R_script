read.table('D:/R (UCL project)/Vortex study copy number new/Outputs/Vortex_HRDgenes_segments_overlaps.txt')
Vortex=read.table('D:/R (UCL project)/Vortex study copy number new/Outputs/Vortex_HRDgenes_segments_overlaps.txt', header = T)
...
##################################
filter(Vortex, total_CN > a_ploidy)

################################
#identify copy number change in 16 HR genes for all samples 
...
#manually add copy number status for VOR78 BRCA1
Vortex[which(Vortex$sample == 'VOR78' & Vortex$gene == 'BRCA1'),]$CN_status = 'gain'

#output dataframe
write.table(Vortex, file = '20HRDgenes/copy_number_change(20HRDgenes).csv', sep = ',', row.names = FALSE)
...
#sort by absolute frequency(overlook samples that LOH dose not exist)
lvls1 = names(sort(table(Vortex_a[Vortex_a$CN_status == "loss of heterozygosity", "sample"])))
ggplot(data = Vortex_a)+
  geom_bar(
    mapping = aes(
      factor(sample, levels = lvls1),
      fill = CN_status),
    position = 'fill'
  )+
  theme(axis.text.x = element_text(angle = 90,vjust =1, hjust=0.75, size = 10))

#sort by relative frequency(use this one)
...
#make stacked barplot, sorting by relative frequency
...
#only barplot with only LOH and homozygous deletion
...
#####################################################
#identify gene with multiple segment in all sample
